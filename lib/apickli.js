const { assoc, Async: Task, compose, constant, curry, Either, equals, fst, isEmpty, ifElse, map, Maybe, nAry, Pair, pick, propOr, propPathOr, setPath } = require('crocks')
...
_.execute = pair =>
  Task.of(pair)
  .map(p => p.extend(_.resolveTemplates))
