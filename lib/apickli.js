...
_.setCookie = cookie => request => {
  const toughCookie = tough.Cookie.parse(cookie)
  request.jar.setCookie(toughCookie, 'https://httpbin.org')
  const cookieString = request.jar.getCookieString('https://httpbin.org')
  return _.setHeader('Cookie', cookieString)(request)
}
