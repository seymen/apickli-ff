...
const isResponseCodeNotEq = expected => response => {
  const actual = R.prop('statusCode', response)
  assert.notEqual(actual, expected, 'Status code =>')
  return response
}
...
_.assertResponseCodeEqual = _.assert(isResponseCodeEq)
_.assertResponseCodeNotEqual = _.assert(isResponseCodeNotEq)
