...
_.assert = f => (...args) => response => {
  ...
  return tap(f(...resolvedArgs)(response))
}
...
const validateResponseBodyWithSchema = schema => response => {
  const body = JSON.parse(R.prop('body', response))
  assert.isTrue(jsonSchemaValidator(schema, { verbose: true })(body))
}
...
_.assertResponseBodyWithSchema = _.assert(validateResponseBodyWithSchema)
