...
Then(/^response header (.+) should not exist$/, function(header, cb) {
  Task.of(this.responsePair)
  .chain(apickli.assertResponseHeaderNotExist(header))
  .fork(
    e => cb(e),
    () => cb()
  )
...
Then(/^response body path (.+) should be of type array$/, function(path, cb) {
  Task.of(this.responsePair)
  .chain(apickli.assertResponseBodyPathIsArray(path))
  .fork(
    e => cb(e),
    () => cb()
  )
...
Then(/^response body path (.+) should be of type array with length (\d*)$/, function(path, len, cb) {
  Task.of(this.responsePair)
  .chain(apickli.assertResponseBodyPathIsArrayWithLength(path, len))
  .fork(
    e => cb(e),
    () => cb()
  )
