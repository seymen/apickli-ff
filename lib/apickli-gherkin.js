...
Given(/^I set (.+) header to (.+)$/, function(headerName, headerValue) {
  this.request = this.request.chain(apickli.setHeader(headerName, headerValue))
})
...
Then(/^response code should be (.+)$/, function(code) {
  apickli.assertResponseCode(code, this.response)
...
Then(/^response body path (.+) should be (((?!of type).+))$/, function(
  ...
  apickli.assertResponseBodyPath(path, value, this.response)
