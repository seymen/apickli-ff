...
Then(/^response code should be (\d+)$/, function(code) {
  return this.response.then(apickli.assertResponseCodeEqual(code))
})

Then(/^response code should not be (\d+)$/, function(code) {
  return this.response.then(apickli.assertResponseCodeNotEqual(code))
})
